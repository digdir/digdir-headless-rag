...
*Definition track.* An agent entity in the config DB has `:agent/id`, `:agent/name`, `:agent/description`, `:agent/instructions`, `:agent/default-skill-graph`, `:agent/allowed-skill-graphs`, `:agent/allowed-dataset-scopes` (`[tenant dataset-config-key]` tuples), `:agent/guardrails` (e.g. `{:citations-required true}`), `:agent/skill-params`, `:agent/enabled?`, timestamps. Validation enforces that skill-graph ids are registered, the default is in the allowed list, and dataset scopes are well-formed (`server/src/digdir/agents/core.clj:115-168`). Built-ins have their skill graphs reconciled against the code definitions on startup (`digdir.boot.agents/seed!`), so a row cannot stay narrower than its definition; other fields are left alone, and a full reset to code is the explicit `seed-builtin-agents!`.

Two of these fields are stored and validated but not read by the runtime. `:agent/instructions` is required by `validate-agent` and reaches no prompt: the agent loop uses its own tuned default (`skills.builtin.agent.loop`) and synthesis falls back to `rag.synthesis/system-prompt-with-date`. `:agent/guardrails` is surfaced through `agents.policy/resolve-execution-policy` but nothing acts on it, as that namespace already notes. What does differentiate an agent is the skill graph it names and its `:agent/skill-params` overrides, which is how `digdir/altinn-docs-tuned` and `builtin/research-assistant-agent` differ from the default.
