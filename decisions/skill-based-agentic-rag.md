...
**Date**: 2026-02-04
**Status**: Adopted
**Decision**: Skill-based agentic RAG with skill graphs as the runtime unit
...
- Skill graphs define how skills are composed and executed at runtime
- Pipelines remain the ingestion/materialization configuration for datasets
- All configuration remains explicit, durable, auditable, and reproducible

### Naming Convention

- **Pipeline** = ingestion/materialization (namespace: `digdir.pipeline.*`)
- **Skill graph** = runtime execution workflow (namespace: `digdir.skills.*`)
- **Skill** = individual composable capability (protocol: `digdir.rag.skills.core`)
...
Claude Agent (with extended thinking)
    ↓
Available Skills (from skill graph config + policy):
...
4. ✅ Create skill executor that loads skill graph config and calls skills
...
- ❌ Migration of existing runtime flows
...
**Deliverables**:
- Migration scripts for existing skill graph configs
...
4. ✅ Skill parameters resolve from skill graph config (via layered inheritance)
...
### 2. **Skill Graph Config Defines Behavior**
- Which skills are available
- Parameters for each skill
- Default composition (skill graph)
...
Current fixed RAG pipeline → Skill graph-based fixed composition → Agent-orchestrated composition
