(ns digdir.skills.enrichment.apply-facts-test
  "Phase D2 — unit coverage for :builtin/enrichment-apply-facts.

   Mirrors apply-phrases-test (same shape, different field shape per
   triple). The row-failure surfacing test catches the silent-success
   regression that Phase C.5 fixed; the doc-num backfill test catches
   the validation-time backfill bug."
  (:require [clojure.test :refer [deftest is testing use-fixtures]]
            [digdir.rag.skills.core :as skills]
            [digdir.rag.typesense :as ts-utils]
            [digdir.skills.enrichment.apply-facts :as af]
...
(def ^:private sample-proposals
  [{:chunk-id "c1"
    :doc-num "42"
    :facts [{:subject "Altinn 3" :predicate "ble lansert" :object "juni 2020"}
            {:subject "Altinn 3" :predicate "erstatter" :object "Altinn 2"}]
    :provenance {:model "gpt-4o"
                 :prompt-hash "abc123"
                 :generated-at-ms 1700000000000}}
   {:chunk-id "c2"
    :doc-num "43"
    :facts [{:subject "Altinn Studio" :predicate "tilbyr" :object "skjemautvikling"}
            {:subject "" :predicate "p" :object "o"}      ;; should be dropped (blank subject)
            {:subject "s" :predicate "p" :object nil}]    ;; should be dropped (nil object)
    :provenance {:model "gpt-4o"
                 :prompt-hash "abc123"
                 :generated-at-ms 1700000000000}}])

(deftest skill-registered
  (testing ":builtin/enrichment-apply-facts is in the skills registry"
    (is (some? (skills/get-skill :builtin/enrichment-apply-facts)))))

(deftest proposals-to-rows-flattens-and-drops-incomplete-triples
  (let [rows (af/proposals->rows sample-proposals)]
    (testing "Each row carries chunk_id, doc_num, triple cells, triple_text, and provenance"
      (is (= 3 (count rows)) "2 from c1 + 1 from c2 (incomplete triples dropped)")
      (is (every? :chunk_id rows))
      (is (every? :doc_num rows))
      (is (every? :subject rows))
      (is (every? :predicate rows))
      (is (every? :object rows))
      (is (every? :triple_text rows))
      (is (every? :model rows))
      (is (every? :prompt_hash rows))
      (is (every? :generated_at rows)))
    (testing "triple_text concatenates the three cells in surface order"
      (is (= "Altinn 3 ble lansert juni 2020"
             (-> rows first :triple_text))))))
...
(deftest chunk-ids-filter-uses-bracket-list
  (testing "Filter clause matches Typesense bracket-list syntax (lesson learned from revert-chunk)"
    (is (= "chunk_id:=[a,b,c]" (af/chunk-ids-filter ["a" "b" "a" "c"]))))
  (testing "Empty input returns nil so the delete call can be skipped"
    (is (nil? (af/chunk-ids-filter [])))
    (is (nil? (af/chunk-ids-filter [nil nil])))))
  ...
  (testing "With :dry-run? true, the skill returns planned rows and never calls Typesense"
    (let [calls (atom [])]
      (with-redefs [ts-utils/make-ts-settings (fn [_]
        ...
        (let [res (af/execute-apply-facts
                   {:inputs {:proposals sample-proposals
                             :collection-name "enrichment_fact_assertions_abc"}
...
(deftest upsert-row-failures-surface-as-exception
  (testing "If Typesense rejects any row, apply-facts throws — must not silently report success"
    (with-redefs [ts-utils/make-ts-settings (fn [_] {:uri "x" :key "k"})
      ...
      (let [thrown (try
        ...
        (is (= :builtin/enrichment-apply-facts
