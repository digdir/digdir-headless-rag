...
(def multi-retrieval-metadata
  {:skill-id :builtin/multi-retrieval
   ...
   :tool-definition multi-retrieval-tool-definition})
...
(defn execute-multi-retrieval
  ...
  (let [{:keys [queries docs-collection chunks-collection phrases-collection]} inputs
        {:keys [total-limit filter-by merge-strategy]} parameters
        ...
        ;; Build attribution
        search-attribution (cond-> {:queries-executed (count queries)
                             ...
                             filter-by
                             (assoc :filter-applied filter-by
