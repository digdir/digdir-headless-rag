(ns digdir.pipeline.ui.skills
  "Skills management UI for admin interface.
...
(e/defn TemplateRow
  "Table row for a single template"
...
(e/defn TemplateDetailModal
  "Modal showing template graph details"
  [template !show-detail]
  (e/client
    (dom/div
      (dom/props {:style modal-backdrop-style})
      ...
      (dom/div
        ...
        (dom/div
          (dom/props {:style {:margin-bottom "1rem"}})
          (dom/strong (dom/text "Pipeline Steps"))
          (dom/div
            (dom/props {:style {:margin-top "0.5rem"}})
      ...
      (dom/div
        ...
        (dom/div
          ...
          (if (empty? templates)
            (dom/p
              (dom/props {:style {:text-align "center" :color "#6b7280" :padding "2rem"}})
              (dom/text "No templates registered."))
            ...
            (dom/table
              ...
              (dom/tbody
                (e/for-by :id [template templates]
                  (TemplateRow template !show-detail))))))

        ;; Detail modal
        (when show-detail
          (TemplateDetailModal show-detail !show-detail))))))
      ...
      (dom/div
        ...
        (dom/div
          ...
          (dom/div
            ...
            ;; Templates tab
            (dom/button
              (dom/props {:style (if (= active-tab :templates)
                                   tab-active-style
                                   tab-style)})
              (dom/text "Templates")
              (let [[tok err] (e/Token (dom/On "click" identity nil))]
                (when tok
                  (reset! !active-tab :templates)
                  (tok))))
